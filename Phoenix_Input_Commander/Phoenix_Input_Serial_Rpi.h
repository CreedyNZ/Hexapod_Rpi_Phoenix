...
//
//Updated: Andrew Creahan to allow serial input from Rpi and read sensors
//
...
public:
  ...
  signed char cmdmode;  // Mode value
  int mode; // Derived from cmdmode to split value into two parts
  int gait; // Derived from cmdmode to split value into two parts
  int p_frnt; 
...
void CommanderInputController::Init(void)
{
  ...
  g_BodyYShift = 0;
  command.p_frnt = 17;
  pinMode(command.p_frnt, INPUT);      // sets the digital pin as input for prox sensor
  command.begin(38400);
...
void CommanderInputController::ControlInput(void)
{
  ...
  //command.SendMsgs((int)Voltage,(int)Temperature1,39);
  if(command.ReadMsgs() > 0){
    ...
    //Switch gaits
      if ((g_InControlState.GaitType != command.gait)
        && abs(g_InControlState.TravelLength.x)<cTravelDeadZone //No movement
        && abs(g_InControlState.TravelLength.z)<cTravelDeadZone 
        && abs(g_InControlState.TravelLength.y*2)<cTravelDeadZone  ) {
        g_InControlState.GaitType = command.gait;                    
        MSound( 1, 50, 2250+(command.gait*10));
        GaitSelect();
        } 
    //    else {
    ...
    //  }
      ...
      //Walking
      command.direction = atan2 (-command.RightV,  -command.RightH) * 57.2958; // arc tangent of y/x converted to degrees
...
//int Commander::SendMsgs(int Volt, int Temp, int Extra){
//  while(Serial.available() > 0){
//    int checksum = (Volt + Temp + Extra);
